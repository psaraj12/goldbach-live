...
static const char* DEFAULT_SIEVE_Q_FILE = "sieve_q_witness.csv";
...
int main(int argc, char** argv) {
    if (argc < 5) {
        ...
        cout << "  --dump-sieve-q <file.csv> Dump ONLY sieve-path witnesses as N,p,q for the full run/resume range\n\n";
        cout << "Example:\n";
        cout << "  " << argv[0]
             << " 4000000000000000000 4001000000000000000 24 44 100000 10000\n";
        cout << "  " << argv[0]
             << " 4000000000000000000 4000010000000000000 24 44 100000 10000 --dump-sieve-q sieve_q_witness.csv\n";
        return 1;
    }
    ...
    bool resuming = false;
    bool dump_sieve_q_enabled = false;
    string sieve_q_file_name = DEFAULT_SIEVE_Q_FILE;

    for (int i = 1; i < argc; ++i) {
        string arg = argv[i];
        if (arg == "--resume") {
            resuming = true;
        } else if (arg == "--dump-sieve-q") {
            dump_sieve_q_enabled = true;
            if (i + 1 < argc) {
                string next = argv[i + 1];
                if (!next.empty() && next[0] != '-') {
                    sieve_q_file_name = argv[++i];
                }
            }
        }
    }
    ...
    if (dump_sieve_q_enabled) {
        sieve_q_dump.open(sieve_q_file_name, ios::out | ios::trunc);
        if (!sieve_q_dump.is_open()) {
            cerr << "ERROR: cannot open sieve-q dump file: " << sieve_q_file_name << "\n";
            return 1;
        }
		sieve_q_dump << "# original_start=" << original_start << "\n";
sieve_q_dump << "# original_end=" << original_end << "\n";
sieve_q_dump << "# adjusted_start=" << start << "\n";
sieve_q_dump << "# adjusted_end=" << end << "\n";
sieve_q_dump << "# p_anchor_limit=" << p_anchor_limit << "\n";
sieve_q_dump << "# qhot_size=" << QHOT_SIZE << "\n";
        sieve_q_dump << "N,p,q\n";
    }
    ...
    if (dump_sieve_q_enabled) {
        cout << "Sieve-Q dump: " << sieve_q_file_name
             << " for ONLY sieve-path witnesses in [" << start << " .. " << end << "] (N,p,q)\n";
    }
                ...
                for (u64 N = blk; N <= blk_end; N += 2ULL) {
                    ...
                    for (u32 pr : base_primes) {
                        ...
                        if (seg.is_prime(q) && is_prime64(q)) {
                            ...
                            if (dump_sieve_q_enabled) {
#pragma omp critical(sieve_q_dump_write)
                                {
                                    sieve_q_dump << N << "," << (u64)pr << "," << q << '\n';
                                }
                            }
...
#pragma omp atomic
        ...
        if (dump_sieve_q_enabled) sieve_q_dump.flush();
        ++ckpt_count;
    ...
    if (dump_sieve_q_enabled) cout << "Sieve-Q:     " << sieve_q_file_name << "\n";
